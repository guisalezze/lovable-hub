...
export default function CopyProjectDetail() {
  ...
  const { data: items, isLoading: itemsLoading, create, updateContent, updateItem, remove: removeItem } = useCopyItems(id);
  ...
  const handleContentSave = useCallback((itemId: string, content: string) => {
    updateContent.mutate({ id: itemId, content });
  }, [updateContent]);

  const handleRestore = (content: string) => {
    if (!versionsItemId) return;
    // Save current version before restoring
    const current = items?.find((i) => i.id === versionsItemId);
    if (current) {
      updateContent.mutate({ id: versionsItemId, content });
  ...
  const renderItems = (type: string, list: typeof criativos) => (
    <div className="space-y-3">
      <div className="flex justify-end">
        <Button size="sm" variant="outline" className="gap-1.5" onClick={() => create.mutate({ type, title: "" })}>
      ...
      {itemsLoading ? (
        ...
        <p className="text-sm text-muted-foreground text-center py-8">Nenhum bloco criado ainda</p>
      ) : (
        list.map((item) => (
          <CopyItemBlock
            key={item.id}
        ...
        <TabsContent value="referencias" className="mt-4">
          <div className="space-y-4">
            ...
            {filesLoading ? (
              ...
              <p className="text-sm text-muted-foreground text-center py-8">Nenhuma referência adicionada</p>
            ) : (
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                {files.map((f) => (
                  <div key={f.id} className="border rounded-lg p-3 space-y-2 group relative">
                    {f.file_type?.startsWith("image/") ? (
                      <img src={f.file_url} alt={f.file_name} className="w-full h-24 object-cover rounded" />
                    ) : (
                      <div className="h-24 flex items-center justify-center">{fileIcon(f.file_type)}</div>
                    )}
